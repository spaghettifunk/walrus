...
GAME_OBJECTS := $(GAME_SOURCES:%.cpp=$(OBJ_DIR)/game/%.o)

# Dependency files are generated by `-MMD -MP`; they make header changes trigger
# the right recompiles.
DEPS := $(ENGINE_OBJECTS:.o=.d) $(ENTRYPOINT_OBJECT:.o=.d) $(GAME_OBJECTS:.o=.d)
...
.PHONY: all game run engine format clean
...
ifeq ($(VULKAN_MISSING),1)
	$(error VULKAN_SDK is not set. Install the Vulkan SDK and run `make VULKAN_SDK=/path/to/VulkanSDK`)
endif
	$(MAKE) $(GAME_BIN)
...
# Link the game executable. It includes game objects, the engine-owned
# entrypoint object, and the dynamic engine library.
$(GAME_BIN): $(GAME_OBJECTS) $(ENTRYPOINT_OBJECT) $(ENGINE_LIB)
	@mkdir -p $(@D)
	$(CXX) $(GAME_CXXFLAGS) $(GAME_OBJECTS) $(ENTRYPOINT_OBJECT) $(ENGINE_LDFLAGS) -o $@ $(VULKAN_LDFLAGS)
